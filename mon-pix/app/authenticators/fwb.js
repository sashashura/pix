...
export default class FwbAuthenticator extends BaseAuthenticator {
  @service session;
  @service location;
  ...
  async authenticate({ code, redirectUri, state, authenticationKey }) {
    ...
    if (authenticationKey) {
      serverTokenEndpoint = `${ENV.APP.API_HOST}/api/fwb/users?authentication-key=${authenticationKey}`;
    } else {
      serverTokenEndpoint = `${ENV.APP.API_HOST}/api/fwb/token`;
      const bodyObject = {
        code,
