...
export default class LoginFwbRoute extends Route {
  @service session;
  @service router;
  @service location;
  ...
  afterModel({ shouldValidateCgu, authenticationKey } = {}) {
    if (shouldValidateCgu && authenticationKey) {
      return this.router.replaceWith('terms-of-service-fwb', { queryParams: { authenticationKey } });
    }
  }

  async _handleCallbackRequest(code, state) {
    try {
      await this.session.authenticate('authenticator:fwb', {
        code,
  ...
  async _handleRedirectRequest() {
    const response = await fetch(
      `${ENV.APP.API_HOST}/api/fwb/auth-url?redirect_uri=${encodeURIComponent(this.redirectUri)}`
    );
