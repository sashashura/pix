...
function translateMessage(locale: $TSFixMe, key: $TSFixMe) {
...
// @ts-expect-error TS(2393): Duplicate function implementation.
function _formatInvalidAttribute(locale: $TSFixMe, {
  attribute,
  message
}: $TSFixMe) {
  if (!attribute) {
...
function _mapToHttpError(error: $TSFixMe) {
...
function handle(request: $TSFixMe, h: $TSFixMe, error: $TSFixMe) {
