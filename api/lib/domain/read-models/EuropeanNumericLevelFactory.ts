...
class EuropeanNumericLevelFactory {
  static buildFromCompetenceMarks(competenceMarks: $TSFixMe) {
    const europeanNumericLevels = [];
    competenceMarks.forEach(({
      competenceCode,
      level
    }: $TSFixMe) => {
...
function _buildEuropeanNumericLevelFromMergedCompetenceMarks({
  ...
  const foundCompetenceMarks: $TSFixMe = [];
  competenceCodesToMerge.forEach((competenceCodeToFind: $TSFixMe) => {
    const foundCompetenceMark = competenceMarks.find(({
      competenceCode
    }: $TSFixMe) => competenceCode === competenceCodeToFind);
