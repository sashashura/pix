...
function _getSkillsFilteredByStatus(knowledgeElements: $TSFixMe, targetSkills: $TSFixMe, status: $TSFixMe) {
  return knowledgeElements
    .filter((knowledgeElement: $TSFixMe) => knowledgeElement.status === status)
    .map((knowledgeElement: $TSFixMe) => knowledgeElement.skillId)
    .map((skillId: $TSFixMe) => targetSkills.find((skill: $TSFixMe) => skill.id === skillId));
}
