...
function sendAccountCreationEmail(email: $TSFixMe, locale: $TSFixMe, redirectionUrl: $TSFixMe) {
  let pixName;
...
function sendVerificationCodeEmail({
  ...
  if (locale === FRENCH_SPOKEN) {
    (options as $TSFixMe).subject = translate({ phrase: 'verification-code-email.subject', locale: 'fr' }, { code });

    (options as $TSFixMe).variables = {
    code,
    homeName: `pix${settings.domain.tldOrg}`,
    homeUrl: `${settings.domain.pix + settings.domain.tldOrg}/fr/`,
    displayNationalLogo: false,
    ...
    (options as $TSFixMe).subject = translate({ phrase: 'verification-code-email.subject', locale: 'en' }, { code });
