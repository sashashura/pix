...
function getPossibleNextChallenges({
  ...
  const challengesWithReward = nonAnsweredChallenges.map((challenge: $TSFixMe) => {
    return {
      challenge,
  ...
  let maxReward = 0;
  const possibleChallenges = challengesWithReward.reduce((acc: $TSFixMe, challengesWithReward: $TSFixMe) => {
...
function getEstimatedLevelAndErrorRate({
  ...
  const samplesWithResults = samples.map((sample: $TSFixMe) => ({
    sample,
    gaussian: null,
  ...
  for (const answer of allAnswers) {
    ...
    latestEstimatedLevel = samplesWithResults.reduce(
      (estimatedLevel: $TSFixMe, {
        sample,
        probability
      }: $TSFixMe) => estimatedLevel + sample * probability,
      0
    );
  }
  ...
  const rawErrorRate = samplesWithResults.reduce(
    (acc: $TSFixMe, {
      sample,
      probability
    }: $TSFixMe) => acc + probability * (sample - latestEstimatedLevel) ** 2,
    0
  );
...
function getNonAnsweredChallenges({
  ...
  const getAnswerSkill = (answer: $TSFixMe) => challenges.find((challenge: $TSFixMe) => challenge.id === answer.challengeId).skill;
  const alreadyAnsweredSkillsIds = allAnswers.map(getAnswerSkill).map((skill: $TSFixMe) => skill.id);

  const isSkillAlreadyAnswered = (skill: $TSFixMe) => alreadyAnsweredSkillsIds.includes(skill.id);
  const filterNonAnsweredChallenge = (challenge: $TSFixMe) => !isSkillAlreadyAnswered(challenge.skill);
...
function _normalizeFieldDistribution(data: $TSFixMe, field: $TSFixMe) {
