...
class CampaignAssessmentCsvLine {
  acquiredBadges: $TSFixMe;
  campaign: $TSFixMe;
  campaignParticipationInfo: $TSFixMe;
  campaignParticipationService: $TSFixMe;
  emptyContent: $TSFixMe;
  organization: $TSFixMe;
  targetProfileWithLearningContent: $TSFixMe;
  targetedKnowledgeElementsByCompetence: $TSFixMe;
  targetedKnowledgeElementsCount: $TSFixMe;
  translate: $TSFixMe;
  constructor({
    organization,
    campaign,
    ...
    this.targetedKnowledgeElementsCount = _.sum(
      _.map(participantKnowledgeElementsByCompetenceId, (knowledgeElements: $TSFixMe) => knowledgeElements.length)
    );
  ...
  _makeEmptyColumns(times: $TSFixMe) {
    return _.times(times, () => this.emptyContent);
  }

  _getStatsForCompetence(competence: $TSFixMe) {
    return {
  ...
  _makeCompetenceColumns() {
    return _.flatMap(this.targetProfileWithLearningContent.competences, (competence: $TSFixMe) => this._makeSharedStatsColumns({
      id: competence.id,
  ...
  _makeSharedColumns() {
    return [
      ...
      ...(this.organization.showSkills
        ? _.map(this.targetProfileWithLearningContent.skills, (targetedSkill: $TSFixMe) => this._makeSkillColumn(targetedSkill))
        : []),
    ];
  }

  _makeYesNoColumns(isTrue: $TSFixMe) {
  ...
  _makeSkillColumn(targetedSkill: $TSFixMe) {
    ...
    if (competenceId in this.targetedKnowledgeElementsByCompetence) {
      knowledgeElementForSkill = _.find(
        ...
        (knowledgeElement: $TSFixMe) => knowledgeElement.skillId === targetedSkill.id
      );
    }
  ...
  _countValidatedKnowledgeElementsForCompetence(competenceId: $TSFixMe) {
  ...
  _getReachedStage() {
    ...
    return this.targetProfileWithLearningContent.reachableStages.filter((stage: $TSFixMe) => masteryPercentage >= stage.threshold)
      .length;
  }
