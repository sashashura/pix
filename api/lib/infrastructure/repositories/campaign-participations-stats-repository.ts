...
const CampaignParticipationsStatsRepository = {
  async getParticipationsActivityByDate(campaignId: $TSFixMe) {
...
async function _getCumulativeParticipationCountsByDay(campaignId: $TSFixMe, column: $TSFixMe) {
  ...
  return data.map(({
    day,
    count
  }: $TSFixMe) => ({ day, count: Number(count) }));
}
