...
module.exports = function buildPixAileProfilev2({
  databaseBuilder
}: $TSFixMe) {
  ...
  const buildCompetenceEvaluation = ({
    competenceId,
    assessmentState
  }: $TSFixMe) => {
  ...
  const buildCompetencePlacement = ({
    ...
    _.map(challengeSkillMap, (challengeSkill: $TSFixMe) => {
      ...
      _.map(challengeSkill.inferredSkillIds, (inferredSkillId: $TSFixMe) => {
        _buildKnowledgeElement({
          competenceId,
          answerId,
