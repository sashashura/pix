...
// @ts-expect-error TS(2451): Cannot redeclare block-scoped variable 'POLE_EMPLO... Remove this comment to see the full error message
...
function _buildParticipationsInDifferentStatus({
  databaseBuilder,
  user
}: $TSFixMe) {
...
function _buildAssessmentParticipations({
  databaseBuilder,
  users
}: $TSFixMe) {
...
function _buildProfilesCollectionParticipations({
  databaseBuilder,
  users
}: $TSFixMe) {
...
function _buildMedNumAssessmentParticipations({
  databaseBuilder
}: $TSFixMe) {
