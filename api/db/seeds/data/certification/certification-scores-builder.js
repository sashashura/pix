...
const {
  ...
  CERTIFICATION_QUICK_ANSWERS_WITH_FOCUSED_OUT_DATA,
...
function certificationScoresBuilder({ databaseBuilder }) {
  ...
  _buildScoreForProSessionToFinalize({ databaseBuilder, createdAt, assessmentId: ASSESSMENT_SUCCESS_IN_PRO_SESSION_TO_FINALIZE_ID });
}
...
function _buildScoreForProSessionToFinalize({ databaseBuilder, createdAt, assessmentId }) {
  _buildAnswers({
    answerData: CERTIFICATION_QUICK_ANSWERS_WITH_FOCUSED_OUT_DATA,
    databaseBuilder,
    assessmentId,
    createdAt,
  });
