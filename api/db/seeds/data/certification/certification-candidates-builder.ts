...
function certificationCandidatesBuilder({
  databaseBuilder
}: $TSFixMe) {
  // Few candidates for the started session
  _.each([CANDIDATE_DATA_SUCCESS, CANDIDATE_DATA_FAILURE, CANDIDATE_DATA_MISSING], (candidate: $TSFixMe) => {
...
// @ts-expect-error TS(7023): '_convertToRoman' implicitly has return type 'any'... Remove this comment to see the full error message
function _convertToRoman(num: $TSFixMe) {
  if (num === 0) {
    return '';
  }
...
function _addCandidateWithTestCompleted({
  sessionId,
  databaseBuilder
}: $TSFixMe) {
...
function _addCandidateWithTestEndedBySupervisor({
  sessionId,
  databaseBuilder
}: $TSFixMe) {
  const user = databaseBuilder.factory.buildUser();
