...
// @ts-expect-error TS(2451): Cannot redeclare block-scoped variable 'certificat... Remove this comment to see the full error message
function certificationScoresBuilder({
  databaseBuilder
}: $TSFixMe) {
...
function _buildAnswers({
  ...
  _.each(answerData, (answerData: $TSFixMe) => {
...
function _addTenSeconds(date: $TSFixMe) {
  const returnedDate = new Date(date);
  returnedDate.setSeconds(returnedDate.getSeconds() + 10);
  return returnedDate;
}

function _buildSuccessScore(databaseBuilder: $TSFixMe, createdAt: $TSFixMe, assessmentId: $TSFixMe) {
  ...
  _.each(CERTIFICATION_SUCCESS_COMPETENCE_MARKS_DATA, (competenceMarkData: $TSFixMe) => {
    databaseBuilder.factory.buildCompetenceMark({ ...competenceMarkData, assessmentResultId, createdAt });
  });
}

function _buildFailureScore(databaseBuilder: $TSFixMe, createdAt: $TSFixMe, assessmentId: $TSFixMe) {
  ...
  _.each(CERTIFICATION_FAILURE_COMPETENCE_MARKS_DATA, (competenceMarkData: $TSFixMe) => {
    databaseBuilder.factory.buildCompetenceMark({ ...competenceMarkData, assessmentResultId, createdAt });
  });
}
