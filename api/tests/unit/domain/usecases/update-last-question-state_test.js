...
describe('Unit | UseCase | update-last-question-state', function () {
  ...
  context('when lastQuestionState is FOCUSEDOUT', function () {
    ...
    it('should early return if challenge is focused but does not equal assessment.lastChallengeId', async function () {
      // Given
    ...
    it('should call assessmentRepository.updateLastQuestionState if challenge is focused and assessment lastChallengeId equals challenge.id', async function () {
      // Given
      challengeRepository.get.withArgs(focusedChallengeId, domainTransaction).resolves(focusedChallenge);
