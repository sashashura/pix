...
describe('Unit | Domain | Models | CertificationAssessment', function () {
  ...
  describe('constructor', function () {
    ...
    _.forIn(CertificationAssessment.states, (value: $TSFixMe) => {
      // @ts-expect-error TS(2582): Cannot find name 'it'. Do you need to install type... Remove this comment to see the full error message
  ...
  describe('#neutralizeChallengeByNumberIfKoOrSkippedOrPartially', function () {
    ...
    certificationAnswersByDate: [
        domainBuilder.buildAnswer({
            ...
            assessmentId: (CertificationAssessment as $TSFixMe).id,
        }),
    ],
});
    ...
    it('should skip unanswered challenges', function () {
      ...
      expect(
        certificationAssessment.certificationChallenges.find(
          (certificationChallenge: $TSFixMe) => certificationChallenge.challengeId === 'rec456'
        ).hasBeenSkippedAutomatically
      ).to.be.true;
      expect(
        certificationAssessment.certificationChallenges.find(
          (certificationChallenge: $TSFixMe) => certificationChallenge.challengeId === 'rec1234'
        ).hasBeenSkippedAutomatically
      ).to.be.false;
      expect(
        certificationAssessment.certificationChallenges.find(
          (certificationChallenge: $TSFixMe) => certificationChallenge.challengeId === 'rec789'
        ).hasBeenSkippedAutomatically
      ).to.be.false;
    });
  });
