      ...
      it('should skip unpassed challenges', async function () {
        ...
        expect(
          certificationAssessment.certificationChallenges.find(
            (certificationChallenge: $TSFixMe) => certificationChallenge.challengeId === 'recChal123'
          ).hasBeenSkippedAutomatically
        ).to.be.true;
        expect(
          certificationAssessment.certificationChallenges.find(
            (certificationChallenge: $TSFixMe) => certificationChallenge.challengeId === 'recChal456'
          ).hasBeenSkippedAutomatically
        ).to.be.false;
      });
    });
