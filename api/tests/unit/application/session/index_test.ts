  ...
  describe('id validation', function () {
    ...
    [
      ...
      it(`should return 400 when ${condition}`, async function () {
        ...
        // when
const response = await httpTestServer.request(request.method, request.url, (request as $TSFixMe).payload || null);
  ...
  describe('GET /api/sessions/{id}/supervising', function () {
    ...
    it('should return 200 if the user is a supervisor of the session and certification center is in the whitelist', async function () {
      //given
      sinon.stub(endTestScreenRemovalEnabled, 'verifyBySessionId').callsFake((request: $TSFixMe, h: $TSFixMe) => h.response(true));
      sinon.stub(sessionSupervisorAuthorization, 'verifyBySessionId').callsFake((request: $TSFixMe, h: $TSFixMe) => h.response(true));
