  ...
  describe('DELETE /api/admin/organizations/{id}/places/{placeId}', function () {
    context('when user is not allowed to access resource', function () {
      it('should resolve a 403 HTTP response', async function () {
        ...
        expect(securityPreHandlers.checkAdminMemberHasRoleCertif).to.not.have.been.called;
        expect(securityPreHandlers.checkAdminMemberHasRoleSupport).to.not.have.been.called;
        expect(response.statusCode).to.equal(403);
      });
    });
  });
