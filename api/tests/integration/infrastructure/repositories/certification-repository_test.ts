...
describe('Integration | Repository | Certification ', function () {
  let sessionLatestAssessmentRejected: $TSFixMe;
  let sessionWithPublishedCertificationCourses: $TSFixMe;
  let sessionWithStartedAndError: $TSFixMe;
  let sessionLatestAssessmentRejectedCertifCourseIds: $TSFixMe;
  let sessionWithStartedAndErrorCertifCourseIds: $TSFixMe;
  let sessionWithPublishedCertificationCoursesCertifCourseIds: $TSFixMe;

  let certificationCenterId: $TSFixMe;
  let certificationCenter: $TSFixMe;
  ...
  describe('#publishCertificationCoursesBySessionId', function () {
    ...
    it('should flag the specified certifications as published', async function () {
      ...
      await Promise.all(
        sessionLatestAssessmentRejectedCertifCourseIds.map(async (id: $TSFixMe) => {
    ...
    it('should not flag the specified certifications as published and be rejected', async function () {
      ...
      await Promise.all(
        sessionWithStartedAndErrorCertifCourseIds.map(async (id: $TSFixMe) => expect((await get(id)).isPublished).to.be.false)
      );
    });
  ...
  describe('#unpublishCertificationCoursesBySessionId', function () {
    ...
    it('should update the specified certifications', async function () {
      ...
      await Promise.all(
        sessionWithPublishedCertificationCoursesCertifCourseIds.map(async (id: $TSFixMe) => {
          const certifCourse = await get(id);
  ...
  function createPublishedCertifCourseWithAssessementResults(sessionId: $TSFixMe, ...assessmentResults: $TSFixMe[]) {
  ...
  function createCertifCourseWithAssessementResults(sessionId: $TSFixMe, ...assessmentResults: $TSFixMe[]) {
