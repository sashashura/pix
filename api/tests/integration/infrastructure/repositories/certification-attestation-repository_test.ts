...
describe('Integration | Infrastructure | Repository | Certification Attestation', function () {
  ...
  describe('#get', function () {
    ...
    it('should throw a NotFoundError when certification attestation does not exist', async function () {
      ...
      expect((error as $TSFixMe).message).to.equal('There is no certification course with id "123"');
    });
...
async function _buildIncomplete(certificationAttestationData: $TSFixMe) {
...
async function _buildRejected(certificationAttestationData: $TSFixMe) {
  databaseBuilder.factory.buildUser({ id: certificationAttestationData.userId });
  const certificationCenterId = databaseBuilder.factory.buildCertificationCenter().id;
  databaseBuilder.factory.buildSession({
    id: certificationAttestationData.sessionId,
...
async function _buildCancelled(certificationAttestationData: $TSFixMe) {
  databaseBuilder.factory.buildUser({ id: certificationAttestationData.userId });
  const certificationCenterId = databaseBuilder.factory.buildCertificationCenter().id;
  databaseBuilder.factory.buildSession({
    id: certificationAttestationData.sessionId,
...
async function _buildValidCertificationAttestation(certificationAttestationData: $TSFixMe, buildCompetenceMark = true) {
  databaseBuilder.factory.buildUser({ id: certificationAttestationData.userId });
  const certificationCenterId = databaseBuilder.factory.buildCertificationCenter().id;
  databaseBuilder.factory.buildSession({
    id: certificationAttestationData.sessionId,
...
async function _buildValidCertificationAttestationWithSeveralResults(certificationAttestationData: $TSFixMe) {
  databaseBuilder.factory.buildUser({ id: certificationAttestationData.userId });
  const certificationCenterId = databaseBuilder.factory.buildCertificationCenter().id;
  databaseBuilder.factory.buildSession({
    id: certificationAttestationData.sessionId,
...
async function _buildCleaResult({
...
async function _buildOtherComplementaryCertifResult({
  certificationCourseId
}: $TSFixMe) {
  const complementaryCertificationCourseId = databaseBuilder.factory.buildComplementaryCertificationCourse({
    certificationCourseId,
...
async function _buildPixPlusDroitMaitreResult({
  certificationCourseId,
  acquired
}: $TSFixMe) {
  const complementaryCertificationCourseId = databaseBuilder.factory.buildComplementaryCertificationCourse({
