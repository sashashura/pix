...
describe('Integration | Infrastructure | Repository | Private Certificate', function () {
  ...
  let pixDroitComplementaryCertificationId: $TSFixMe, pixEduComplementaryCertificationId: $TSFixMe;
  ...
  describe('#get', function () {
    ...
    it('should throw a NotFoundError when private certificate does not exist', async function () {
      ...
      expect((error as $TSFixMe).message).to.equal('Certificate not found for ID 123');
    });
...
async function _buildValidPrivateCertificate(privateCertificateData: $TSFixMe, buildCompetenceMark = true) {
  const certificationCenterId = databaseBuilder.factory.buildCertificationCenter().id;
  const sessionId = databaseBuilder.factory.buildSession({
    publishedAt: privateCertificateData.deliveredAt,
    certificationCenter: privateCertificateData.certificationCenter,
...
async function _buildValidPrivateCertificateWithAcquiredAndNotAcquiredBadges({
  ...
  temporaryAcquiredBadges?.forEach(({
    key,
    imageUrl,
    complementaryCertificationId
  }: $TSFixMe) => {
...
async function _buildValidPrivateCertificateWithSeveralResults(privateCertificateData: $TSFixMe) {
  const certificationCenterId = databaseBuilder.factory.buildCertificationCenter().id;
  const sessionId = databaseBuilder.factory.buildSession({
    publishedAt: privateCertificateData.deliveredAt,
    certificationCenter: privateCertificateData.certificationCenter,
