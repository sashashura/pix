...
async function _buildDomainAndDatabaseBadge(key: $TSFixMe, targetProfileId: $TSFixMe) {
...
async function buildDomainAndDatabaseStage(targetProfileId: $TSFixMe) {
...
const checkBadge = (actual: $TSFixMe, expected: $TSFixMe) => {
...
describe('Integration | Repository | Target-profile-with-learning-content', function () {
  ...
  describe('#get', function () {
    ...
    it('should return target profile badges without imageUrl', async function () {
      ...
      // then
      const actualBadge1 = targetProfile.badges.find((badge: $TSFixMe) => badge.id === badge1.id);
      checkBadge(actualBadge1, {
        ...badge1,
        imageUrl: null,
      });
      const actualBadge2 = targetProfile.badges.find((badge: $TSFixMe) => badge.id === badge2.id);
