...
describe('Integration | Infrastructure | Repository | Shareable Certificate', function () {
  ...
  let pixCleaComplementaryCertificationId: $TSFixMe, pixDroitComplementaryCertificationId: $TSFixMe, pixEduComplementaryCertificationId: $TSFixMe;
  ...
  describe('#getByVerificationCode', function () {
    ...
    it('should throw a NotFoundError when shareable certificate does not exist', async function () {
      ...
      expect((error as $TSFixMe).message).to.equal('There is no certification course with verification code "P-SOMECODE"');
    });
...
async function _buildValidShareableCertificate(shareableCertificateData: $TSFixMe, buildCompetenceMark = true) {
  const certificationCenterId = databaseBuilder.factory.buildCertificationCenter().id;
  const sessionId = databaseBuilder.factory.buildSession({
