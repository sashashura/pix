...
describe('Integration | Scripts | create-assessment-campaigns', function () {
  ...
  describe('#checkData', function () {
    ...
    it('should throw an error if targetProfileId is missing', async function () {
      ...
      expect((error as $TSFixMe).message).to.equal('Un targetProfileId est manquant pour la campagne SomeName.');
    });
    ...
    it('should throw an error if campaign name is missing', async function () {
      ...
      // then
      expect(error).to.be.instanceOf(Error);
      expect((error as $TSFixMe).message).to.equal('Un nom de campagne est manquant pour le profil cible 123.');
    });
    ...
    it('should throw an error if externalId is missing', async function () {
      ...
      // then
      expect(error).to.be.instanceOf(Error);
      expect((error as $TSFixMe).message).to.equal('Un externalId est manquant pour le profil cible 123.');
    });
    ...
    it('should throw an error if creatorId is missing', async function () {
      ...
      // then
      expect(error).to.be.instanceOf(Error);
      expect((error as $TSFixMe).message).to.equal('Un creatorId est manquant pour le profil cible 123.');
    });
  });
